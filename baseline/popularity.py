...
from eval import precision_at_k, ndcg_at_k
from eval import calc_scores_per_user, prep_for_evaluation, aggregate_results

REPO_ROOT = Path(__file__).resolve().parents[1]  # popularity.py is likely under baseline/
def p(*parts) -> Path:
...
class PopularityBL():
    ...
    def calculate_popularity(self, data_ix_path, user_limit):
        with open(os.path.join(data_ix_path, 'mf',
                               f'processed_train_w_negatives_full_{user_limit}.pkl'), 'rb') as handle:
    ...
    def predict(self,processed_test=pd.DataFrame(),data_dir=None,test_file_path=None):
        ...
        # Debugging statement to check if self.items_score is None
        if self.items_score is None:
            raise ValueError("self.items_score is None. Ensure calculate_popularity is called before predict.")
