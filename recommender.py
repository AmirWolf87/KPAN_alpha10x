...
def load_data(item_person, person_item, user_item_all, item_user_all, item_user_split, user_item_split, neg_samples,
    ...
    print("line 179 - Path file: ", path_file )
    # save all paths
    if save_dicts:
    ...
    for user,pos_items in tqdm(list(user_item_split.items())[:limit]):
        ...
        for pos_item in pos_items:
            ...
            for i in range(neg_samples):
                ...
                print("num_neg_interactions: " , num_neg_interactions)
                cur_index += 1
...
def main(files_loc, params={}, data_loc=None):
    ...
    with open(os.path.join(PROCESSED_DATA_DIR,consts.ITEM_IX_DATA_DIR, 'items_for_pred_alpha10x.pkl'), 'rb') as handle:
...
if __name__ == "__main__":
    main(files_loc=Path(os.getcwd()), data_loc=os.path.join(str(Path(os.getcwd()).parents[0]), 'KPAN_V2'))
