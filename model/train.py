...
def train(model, train_path_file, batch_size, epochs, model_path, load_checkpoint, not_in_memory, lr, l2_reg, gamma,
    ...
    print("Training file:", os.path.basename(train_path_file))
    ...
    epoch_loss = []
    epoch_val_loss = []
    # Initialize a list to store the paths and scores
    # path_scores = []
    # New list to store path attention data
    path_scores_dict = {}
    ...
    for epoch in range(epochs):
        ...
        if model_name == 'KPAN' and epoch == 3:
        ...
        for interaction_batch_raw, targets in tqdm(train_loader):  # have tqdm here when not on colab
            ...
            for i in range(len(interaction_batch)):
                ...
                if path_aggregation == 'weighted_pooling':
                    agg_score = model.weighted_pooling(tag_scores[inter_idxs], gamma=gamma)  # shape [2] or [C]
                elif path_aggregation == 'attention':
                    agg_score, weight_att = model.calc_paths_attention(
                        tag_scores[inter_idxs],
                        s_path_batch[inter_idxs][0][0][0].to(device),
                        s_path_batch[inter_idxs][0][-1][0].to(device)
                    )

                    # store attention info per path (keep only if you really need it)
                    for path_idx in inter_idxs.unique():
                        path_for_csv = s_path_batch[path_idx]
                        path_key = (
                            path_for_csv[0][0].item(), path_for_csv[0][1].item(),
                            path_for_csv[1][0].item(), path_for_csv[1][1].item(),
                            path_for_csv[2][0].item(), path_for_csv[2][1].item(),
                            path_for_csv[-1][0].item(), path_for_csv[-1][1].item(),
                        )
                        path_index = path_idx.item() % weight_att[0][0].size(0)
                        attention_weight = weight_att[0][0][path_index]
                        path_scores_dict[path_key] = (attention_weight.item(), agg_score.item())

                if start_pool:
                    pooled_scores = agg_score.unsqueeze(0)  # [1, C]
                    start_pool = False
                else:
                    pooled_scores = torch.cat((pooled_scores, agg_score.unsqueeze(0)), dim=0)  # [B, C]
        ...
        print(f'finish epoch number {epoch+1} out of {epochs}: {timedelta(seconds=(time.time()) - start_epoch)}')
    ...
    out_dir = Path(model_path).resolve().parent
    out_csv = out_dir / "attention_scores.csv"

    with open(out_csv, "w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow([
            'init_investor', 'init_investor_type', 'inter_org', 'inter_org_type',
            'inter_inv_or_bizperson', 'inter_inv_bizperson_type', 'end_org', 'end_org_type',
            'Attention_Weight', 'Path_Score'
        ])
        for path_key, (weight, score) in path_scores_dict.items():
            writer.writerow(list(path_key) + [weight, score])

    print("Saved attention scores to:", out_csv)
