...
class KPAN(KPRN):
    ...
    def initial_embeddings_mf(self, mf_path, person_item):
        if os.path.isfile(
            ...
            with open(os.path.join(mf_path, f'mf_full_6040_comp_{consts.ENTITY_EMB_DIM}_items_mapping.pkl'), 'rb') as handle:
                mf_items_mapping = pickle.load(handle)
            with open(os.path.join(mf_path, f'mf_full_6040_comp_{consts.ENTITY_EMB_DIM}_users_mapping.pkl'), 'rb') as handle:
                mf_users_mapping = pickle.load(handle)
            with open(os.path.join(mf_path, f'mf_full_6040_comp_{consts.ENTITY_EMB_DIM}_items_embeddings.pkl'), 'rb') as handle:
                mf_items_embeddings = pickle.load(handle).T
            with open(os.path.join(mf_path, f'mf_full_6040_comp_{consts.ENTITY_EMB_DIM}_users_embeddings.pkl'), 'rb') as handle:
