...
class KPAN(KPRN):
    ...
    def initial_embeddings_mf(self, mf_path, person_item):
        # Check if the precomputed MF files exist
        if os.path.isfile(os.path.join(mf_path, f'mf_full_type_embeddings_{consts.TYPE_EMB_DIM}_as_array.pkl')):
            # Load the precomputed embeddings
            with open(os.path.join(mf_path, f'mf_full_embeddings_{consts.ENTITY_EMB_DIM}_as_array.pkl'),
                      'rb') as handle:
                embedding_as_array = pickle.load(handle)
            with open(os.path.join(mf_path, f'mf_full_type_embeddings_{consts.TYPE_EMB_DIM}_as_array.pkl'),
                      'rb') as handle:
                type_embedding_as_array = pickle.load(handle)
            print('Precomputed MF embeddings loaded.')
            ...
            print('Precomputed MF embeddings NOT found - computing them as fallback.')
            # Load the mapping files from the MF pipeline
            with open(os.path.join(mf_path, f'mf_full_6040_comp_{consts.ENTITY_EMB_DIM}_items_mapping.pkl'),
                      'rb') as handle:
                mf_items_mapping = pickle.load(handle)
            with open(os.path.join(mf_path, f'mf_full_6040_comp_{consts.ENTITY_EMB_DIM}_users_mapping.pkl'),
                      'rb') as handle:
                mf_users_mapping = pickle.load(handle)
            with open(os.path.join(mf_path, f'mf_full_6040_comp_{consts.ENTITY_EMB_DIM}_items_embeddings.pkl'),
                      'rb') as handle:
                mf_items_embeddings = pickle.load(handle).T
            with open(os.path.join(mf_path, f'mf_full_6040_comp_{consts.ENTITY_EMB_DIM}_users_embeddings.pkl'),
                      'rb') as handle:
                mf_users_embeddings = pickle.load(handle)

            # Define a helper function to update embeddings
            def update_embedding(df_mapping, df_embedding, embedding_to_update):
                print(f'df_embedding shape: {df_embedding.shape}')
                print(f'embedding_to_update shape: {embedding_to_update.shape}')
            ...
            # Initialize a zero array with the same shape as current entity embeddings
            embedding_as_array = np.zeros_like(np.array(self.entity_embeddings.weight.data))
            # Update items and users embeddings using the helper function
            update_embedding(mf_items_mapping, mf_items_embeddings, embedding_as_array)
            update_embedding(mf_users_mapping, mf_users_embeddings, embedding_as_array)

            # For persons, use the average of their associated items' embeddings
            ...
            # Now update type embeddings similarly
            with open(os.path.join(mf_path, f'mf_full_6040_comp_{consts.TYPE_EMB_DIM}_items_mapping.pkl'),
                      'rb') as handle:
                type_mf_items_mapping = pickle.load(handle)
            with open(os.path.join(mf_path, f'mf_full_6040_comp_{consts.TYPE_EMB_DIM}_items_embeddings.pkl'),
                      'rb') as handle:
                type_mf_items_embeddings = pickle.load(handle).T
            with open(os.path.join(mf_path, f'mf_full_6040_comp_{consts.TYPE_EMB_DIM}_users_embeddings.pkl'),
                      'rb') as handle:
            ...
            # Set type embeddings for items and users as the mean over the respective MF embeddings
            ...
            # For persons, average the embeddings of items associated with each person
                ...
                for item in person_item[person]:
                    ...
                    if type_mf_items_mapping[type_mf_items_mapping.item == item]['item_idx'].shape[0] > 0:
                        items_to_calc.append(
                            type_mf_items_mapping[type_mf_items_mapping.item == item]['item_idx'].values[0])
            ...
            # Save the computed type embeddings
            with open(os.path.join(mf_path, f'mf_full_type_embeddings_{consts.TYPE_EMB_DIM}_as_array.pkl'),
                      'wb') as handle:
    ...
    def calc_paths_attention(self, path_scores,user,item):
        ...
        max_capacity = self.attention_model.embed_dim  # Or some other measure of the capacity
        num_paths = key_value.size(0)
        if num_paths > max_capacity:
            key_value = key_value[:max_capacity]
        paths_att,weight_att = self.attention_model(query_dim,key_value,key_value,need_weights=True)
        # Handle cases where num_paths exceeds capacity
        if num_paths > max_capacity:
            weight_att = torch.cat([weight_att, torch.zeros(1, 1, num_paths - max_capacity)], dim=2)
