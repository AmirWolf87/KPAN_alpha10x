...
def g_at_k_per_pos(ranked_tuples, k):
    '''
...
def mpr(ranked_df):
    ...
    if n <= 1:
        return 0.0
    denom = (n - 1)
    return float((ranked_df['label'] * ((ranked_df['rank'] - 1) / denom)).sum() / ranked_df['label'].sum())
...
def calc_scores_per_user(df, max_k, model_nm, mpr_metric=False):
    ...
    for user in list(df.index.unique()):
        ...
        scores_per_user.extend(list(zip([model_nm] * max_k,
                                        ...
                                        precision,
                                        recall)))
        mpr_per_user = pd.concat([mpr_per_user, pd.DataFrame({'user': [int(user)], 'mpr': [mpr(cur_user)]})], ignore_index=True)
